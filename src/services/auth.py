...
class AuthService:
    ...
    async def get_token(self, form_data: OAuth2PasswordRequestForm) -> Token:
        ...
        if (
            not user
            or not self.__verify_password(plain_password=form_data.password, hashed_password=user.hashed_password)
        ):
            raise credentials_exception

        access_token_expires = timedelta(minutes=settings().ACCESS_TOKEN_EXPIRE_MINUTES)
        access_token = self.__create_access_token(data={"sub": user.username}, expires_delta=access_token_expires)
    ...
    def __create_access_token(self, data: dict[str, str], expires_delta: timedelta | None = None) -> str:
        to_encode = data.copy()
        if expires_delta:
            expire = datetime.now(timezone.utc) + expires_delta
        else:
            expire = datetime.now(timezone.utc) + timedelta(minutes=15)
        timestamp = int(expire.replace(microsecond=0).timestamp())

        to_encode.update({"exp": str(timestamp)})
